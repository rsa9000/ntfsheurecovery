...
usage () {
	APPNAME=$(basename "$0")
	echo "ntfsheurecovery orphaned clusters type detector

This script analyses DB clusters map and call file(1) utility for each
orphaned cluster.
...
sqlite3 $DB "$Q" | while read R; do
	...
	L=$(($F + $N - 1))
	for I in $(seq $F $L); do
		FTYPE=$(dd if=$IMG bs=$VOL_CLS_SZ count=1 skip=$I 2>/dev/null | file - | sed -e "s/^\/dev\/stdin: //")
		[ "$FTYPE" == "data" ] && continue
		echo "$I: $FTYPE"
	done
done
